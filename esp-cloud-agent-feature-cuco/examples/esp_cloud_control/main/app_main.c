...
#include <esp_cloud_diagnostics.h>
#include <esp_cloud_user_assoc.h>
#include "app_priv.h"

static const char *TAG = "app_main";
static esp_cloud_handle_t g_esp_cloud_handle;
                               ...
                               wifi_prov_cb_event_t event, void *event_data)
{
    switch (event) {
        case WIFI_PROV_START:
            ...
            esp_cloud_user_assoc_endpoint_register();
            break;
...
static void initialise_wifi(void)
{
    ...
    if (!provisioned) {
        ...
        esp_cloud_user_assoc_endpoint_create();
...
static char *data_5min = "5_min";
static char *data_8min = "8_min";
/* Random periodic diagnostics handler */
static void diag_handler(esp_cloud_handle_t handle, void *priv_data)
{
    char data[50];
    snprintf(data, sizeof(data), "{\"info\":\"%s\"}", (char *)priv_data);
    ESP_LOGI(TAG, "Sending: %s", data);
    esp_cloud_diagnostics_send_data(handle, data);
}

static char *my_priv_data = "my_priv_data";
static esp_err_t output_callback(const char *name, esp_cloud_param_val_t *param, void *priv_data)
{
    ...
    ESP_LOGI(TAG, "Received value = %s for %s", param->val.b? "true" : "false", name);
    if (priv_data) {
...
void app_main()
{
    ...
    ESP_LOGI(TAG, "Connected to WiFi network! Starting ESP Cloud Agent...");
    ...
    esp_cloud_diagnostics_register_periodic_handler(g_esp_cloud_handle, diag_handler, 8 * 60, data_8min);
    esp_cloud_start(g_esp_cloud_handle);

    static char data[50];
    snprintf(data, sizeof(data), "{\"reboot_reason\":%d}", esp_reset_reason());
    esp_cloud_diagnostics_add_data(g_esp_cloud_handle, data, false);
}
